...
**오더 중복 기준·수동 오더 삭제 (2026-09-15, `0052`)** — 오더 중복은 **품목코드·제조번호·품목명·구분 네 값이 모두 같을 때만**이다(`uq_pct_orders_identity`, 구분 null=''·앞뒤 공백 무시, 삭제된 수동 오더는 제외). 하나라도 다르면 같은 제조번호·품목코드의 자동 오더가 있어도 수동 오더를 등록·수정할 수 있다. 자동 적재 오더끼리는 예전처럼 (제조번호, 품목코드) 유일(`uq_pct_orders_auto_batch_code`) — 시트 적재 자연키. 적재(`pctIngest.ts`)는 기존 오더 매칭에서 수동 오더를 빼 수동 오더를 덮어쓰지 않고, 네 값이 모두 같은 수동 오더와 부딪힌 시트 행은 실패로 기록한다. AI 자동배정은 엔진 결과를 오더 id 로 되붙인다(`scheduleEngine` 행·결과의 `orderId`). 같은 키 오더가 둘 이상이면 카드에 구분 배지를 늘 띄운다. 23505 문구는 인덱스 이름으로 가른다(`pctOrders.duplicateOrderMessage`, 0052 적용 전이면 적용 안내). 규칙: `intent/2026-09-15-order-duplicate-key-spec.md`.
수동 오더는 오더 수정 서랍 하단 [취소] 왼쪽 **[삭제]**(관리자)로 지운다 — `DELETE /api/pct-orders {id, reason}` → `pctOrders.deleteManualOrder`: 사유 필수, 소프트 삭제(status '삭제'·deleted_at, ingest_state 'manual' 유지), `pct_order_edits` 이력 먼저, 동시분석 그룹에서 제외(남은 멤버 1건 이하면 그룹 해체), 담당자 구성은 유지(적재 삭제와 같음). 작업이 시작됐거나 확정(LOCK)된 오더·자동 오더는 거절.
