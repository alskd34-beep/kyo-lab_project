/**
 * ⚠️ 2026-08-22 점검 — localStorage 영속화는 임시 방편이다(정리 대상).
 *
 * 배정 결과가 브라우저에만 남아 다른 사용자/기기에서 보이지 않고,
 * 서버에 이력이 남지 않아 감사추적(GMP)에 쓸 수 없다.
 * 이관 대상 테이블은 `ai_weekly_schedules` (마이그레이션 0028)이며,
 * 조회 API 는 이미 `/api/schedules/monthly` 로 존재한다.
 * 배경: docs/system-audit-2026-08-22.md 6번 항목.
 */
/**
 * PCT 생산관리 → 월간 스케줄 브릿지
 *
