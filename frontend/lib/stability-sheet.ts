...
export { stabilityCompositeKey } from "@shared/stability"
...
export function stabilitySourceKey(
  row: Pick<
    StabilitySheetRow,
    "requestNo" | "productCode" | "batchNo" | "testType" | "period"
  >
): string {
  const clean = (value: string) => value.trim().replace(/\s+/g, " ")
  return row.requestNo.trim()
    ? `request:${clean(row.requestNo)}`
    : stabilityCompositeKey(row)
}
...
export function mapRow(
  ...
  const period =
    pick(row, ["안정성시험 계획 정보/기간", "진행정보/기간", "기간"]) ||
    [start, end].filter(Boolean).join(" ~ ")
  const productCode = pick(row, [
    "안정성시험 계획 정보/품목코드",
    "품목코드",
    "자재코드",
  ])
  const batchNo = pick(row, [
    "안정성시험 계획 정보/제조번호",
    "제조번호",
    "Lot",
    "Lot No",
  ])
  const requestNo = pick(row, [
    "진행정보/의뢰번호",
    "상세정보/의뢰번호",
    "의뢰번호",
    "외뢰번호",
...
export function isStabilitySummaryRow(row: StabilitySheetRow): boolean {
  return Object.entries(row.source).some(([key, value]) => {
    ...
    return (
      normalizedKey.includes("목록번호") && /^총건수:?$/.test(normalize(value))
    )
  })
}

export function getStabilityStatusClass(
  status: string,
  approved = true
): string {
  const value = normalize(status)
  if (!approved || /(미승인|미확인)/.test(value))
    return "border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-300"
  if (/(완료|종료|승인)/.test(value))
    return "border-blue-400 bg-blue-100 text-blue-900 dark:border-blue-600 dark:bg-blue-900/60 dark:text-blue-200"
  if (/(진행|시험중|분석중|의뢰)/.test(value))
    return "border-blue-200 bg-blue-50 text-blue-700 dark:border-blue-800 dark:bg-blue-950 dark:text-blue-300"
  if (/(대기|예정|준비)/.test(value))
    return "border-amber-200 bg-amber-50 text-amber-700 dark:border-amber-800 dark:bg-amber-950 dark:text-amber-300"
  if (/(보류|지연|중단|취소)/.test(value))
...
export function isScheduleCandidate(row: StabilitySheetRow): boolean {
  return (
    !!row.productCode && !/(완료|종료|취소|중단)/.test(normalize(row.status))
  )
}
