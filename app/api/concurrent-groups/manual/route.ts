 ...
 *   POST /api/concurrent-groups/manual            { orderIds, label?, note? } — 동시분석 그룹 지정 (admin)
 *   POST /api/concurrent-groups/manual?preview=1  { orderIds }                — 검증만(쓰기 없음)
 *
 * 동시분석 그룹 지정은 관리자의 판단이 규칙보다 우선한다는 전제다. 그래서 대부분의 문제는 경고로만
...
export async function POST(req: NextRequest) {
  ...
  try {
    const body = await req.json() as { orderIds?: string[]; label?: string; note?: string; representativeOrderId?: string | null }
    const orderIds = Array.isArray(body.orderIds) ? body.orderIds.filter(Boolean) : []
    if (orderIds.length === 0) {
      return Response.json({ error: '동시분석 그룹으로 지정할 오더를 선택해 주세요.' }, { status: 400 })
    }
    ...
    const result = await createManualGroup(
      orderIds, { label: body.label ?? null, note: body.note ?? null, representativeOrderId: body.representativeOrderId ?? null }, auth.payload.sub ?? null,
    )
