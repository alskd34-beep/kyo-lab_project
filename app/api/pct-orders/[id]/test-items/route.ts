/**
 * [BACKEND] 오더별 시험항목 — 품목 기준 스냅샷 위에서 개별 제외/추가 (0029)
 *
...
import {
  listOrderDetail, setExcluded, addManualItem, removeItem,
...
export async function PATCH(req: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  ...
  try {
    const { id } = await ctx.params
    const body = await req.json() as { testItemName?: string; excluded?: boolean; reason?: string }
    if (!body.testItemName || typeof body.excluded !== 'boolean') {
